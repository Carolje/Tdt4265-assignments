...
from ssd.modeling.backbones import basic_augmented
from tops.config import LazyCall as L
from ssd.data.mnist import MNISTDetectionDataset
from ssd import utils
from ssd.data.transforms import Normalize, ToTensor, GroundTruthBoxesToAnchors,RandomHorizontalFlip,ColorJitter
from ssd.data.transforms.gpu_transforms import RandomRotation
...
anchors = L(AnchorBoxes)(
    feature_sizes=[[75,75],[38, 38], [19, 19], [10, 10], [5, 5], [3, 3], [1, 1]],
    # Strides is the number of pixels (in image space) between each spatial position in the feature map
    strides=[[4,4],[8, 8], [16, 16], [32, 32], [64, 64], [100, 100], [300, 300]],
    min_sizes=[[15,15],[30, 30], [60, 60], [111, 111], [162, 162], [213, 213], [264, 264], [315, 315]],
...
backbone = L(basic_augmented.BasicModelAugmented)(
...
data_train = dict(
    ...
    augmented_dataset=L(MNISTDetectionDataset)(
        data_dir=get_dataset_dir("mnist_object_detection/train"),
        is_train=True,
        transforms=L(torchvision.transforms.Compose)(
        transform=[L(ToTensor)(),
    ...
    augmented_dataloader=L(torch.utils.data.DataLoader)(
        dataset="${..augmented_dataset}", num_workers=4, pin_memory=True, shuffle=True, batch_size="${...train.batch_size}", collate_fn=utils.batch_collate,
        drop_last=True
    ),
    ...
    dataloader_augment=L(torch.utils.data.ConcatDataset)(
        dataset="${..dataloader}",augmented_dataset="${..augmented_dataloader}"
    )
)
data_val = dict(
    dataset=L(MNISTDetectionDataset)(
