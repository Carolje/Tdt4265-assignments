...
def main():
    ...
    w = [model.w, model1.w]
    fig, axs = plt.subplots(2,10,figsize = (10,4), sharex=True, sharey=True) # figsize = (10,4)
    for j in range(2):
        for i in range(10):
            axs[j,i].imshow(np.reshape(w[j][1:,i].T, (28,28)), cmap="gray")
            axs[j,i].axis("off")
    plt.subplots_adjust(wspace=0, hspace=0)
    plt.savefig("task4b_softmax_weight.png", bbox_inches='tight')
    plt.show()
    ...
    for lamb in l2_lambdas:
        model = SoftmaxModel(l2_reg_lambda=lamb)
        trainer = SoftmaxTrainer(
            model, learning_rate, batch_size, shuffle_dataset,
            X_train, Y_train, X_val, Y_val,)
